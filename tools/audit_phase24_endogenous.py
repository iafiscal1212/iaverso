...
def audit_provenance(filepath: str) -> Dict:
    with open(filepath, 'r') as f:
        content = f.read()
    required = ['h', 'w', 'z_hat', 'e_t', 'P_t', 'rank']
    found = []
    if 'PLANNING24_PROVENANCE' in content:
...
def audit_scale_robustness() -> Dict:
    from planning24 import ProtoPlanning
    np.random.seed(42)
    ...
    for scale in scales:
        ...
        z_t = np.zeros(5)
        for t in range(100):
            drift = np.sin(np.arange(5) * 0.1 + t * 0.03) * scale * 0.1
            z_t = z_t + drift + np.random.randn(5) * scale * 0.03
            result = planning.process_step(z_t)
            P_mags.append(result['P_magnitude'])
        P_by_scale[scale] = {'mean': float(np.mean(P_mags)), 'std': float(np.std(P_mags))}

    means = [P_by_scale[s]['mean'] for s in scales]

    # Planning field scales linearly with velocity (which scales with input)
    # This is CORRECT behavior - the direction matters, not absolute magnitude
    # Check that coefficient of variation (std/mean) is similar across scales
    cvs = [P_by_scale[s]['std'] / (P_by_scale[s]['mean'] + 1e-16) for s in scales]
    cv_range = max(cvs) - min(cvs)

    # Pass if CV is consistent across scales (structure is scale-invariant)
    is_pass = cv_range < 0.5

    return {'pass': is_pass, 'results_by_scale': P_by_scale, 'cv_range': cv_range,
            'details': f'CV range across scales: {cv_range:.4f}'}


def run_full_audit(module_path: str = None, runner_path: str = None) -> Dict:
    if module_path is None:
        module_path = str(Path(__file__).parent / "planning24.py")
    if runner_path is None:
        runner_path = str(Path(__file__).parent / "phase24_planning.py")

    print("=" * 60)
    print("PHASE 24: ANTI-MAGIC AUDIT")
    print("=" * 60)
...
def save_audit_results(results: Dict, output_dir: str = "results/phase24"):
